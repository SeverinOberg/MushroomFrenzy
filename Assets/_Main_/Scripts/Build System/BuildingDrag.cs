...
public class BuildingDrag : MonoBehaviour 
{
    ...
    private bool Build()
    {
        ...
        if (!owner.ResourceManager.PayForBuild(building.buildingSO))
        {
            return false;
        }
    ...
    {
        if (!Build() || !owner.ResourceManager.HasSufficientResourcesToBuild(building.buildingSO, out _))
        {
            return; 
        }

        owner.BuildingSystem.InitializeWithObject(building.Prefab);
    }
